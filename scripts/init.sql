...
CREATE TYPE membership_status AS ENUM ('pending', 'active', 'expired', 'frozen');
END IF;
...
ALTER TABLE discounts ADD COLUMN IF NOT EXISTS name varchar NOT NULL DEFAULT 'Promoción';
...
END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_discount_plans_discount' AND table_name = 'discount_plans' AND table_schema = 'public') THEN
ALTER TABLE discount_plans ADD CONSTRAINT fk_discount_plans_discount FOREIGN KEY (discount_id) REFERENCES discounts (id) ON DELETE CASCADE;
END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_discount_plans_plan' AND table_name = 'discount_plans' AND table_schema = 'public') THEN
ALTER TABLE discount_plans ADD CONSTRAINT fk_discount_plans_plan FOREIGN KEY (plan_id) REFERENCES plans (id) ON DELETE CASCADE;
END IF;
